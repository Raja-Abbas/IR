...
const handleCancel = (boxIndex, itemIndex) => {
  ...
  return (
    <div className='flex justify-around'>
      <div>
        <div className="w-[480px] opacity-60 text-neutral-900 text-[20px] font-normal font-Poppins leading-relaxed">
        ...
        <div
          className='w-[550px] mt-10 flex justify-start flex-wrap gap-1'
        >
      ...
      <div className='flex flex-col gap-6'>
        {[0, 1, 2, 3].map((boxIndex) => (
          <div
            ...
            className="w-[477px] h-[74px] bg-neutral-300 rounded-[20px] flex justify-around items-center"
          >
            {droppedItems
              .filter((item) => item.boxIndex === boxIndex)
              .map((droppedItem, index) => (
                <div key={index} className='h-[30px] px-3 rounded-[18px] border border-indigo-600 justify-center items-center gap-2 inline-flex text-indigo-600 text-xs font-medium font-Poppins leading-3'>
                  {droppedItem.item}
                  <button onClick={() => handleCancel(boxIndex, index)}>&#10005;</button>
                </div>
              ))}
          </div>
        ))}
